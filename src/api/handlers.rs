...
pub async fn get_dashboard(
    ...
    let recent_meetings = meeting_rows
        ...
        .map(|row| {
            ...
            let scheduled_start_at: Option<DateTime<Utc>> = row.get("scheduled_start_at");
            let scheduled_end_at: Option<DateTime<Utc>> = row.get("scheduled_end_at");
...
pub async fn delete_group(
    ...
    sqlx::query("DELETE FROM notifications WHERE related_group_id = $1")
        .bind(parsed_group_id)
        .execute(&mut *tx)
        .await
