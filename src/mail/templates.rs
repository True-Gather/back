...
/// Template d'email de vérification d'adresse email lors de l'inscription.
///
/// Contient un bouton + lien valable 15 minutes.
pub fn email_verification_template(username: &str, verify_url: &str) -> String {
    format!(
        r#"<!DOCTYPE html>
<html lang="fr">
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>Vérifiez votre email — TrueGather</title>
  <style>
    ...
    .header {{ background-color: #14b8a6; padding: 32px 40px; text-align: center; }}
    .header h1 {{ color: #ffffff; font-size: 22px; margin: 0; }}
    .body {{ padding: 32px 40px; color: #374151; line-height: 1.6; }}
    .body p {{ margin: 0 0 16px; }}
    .btn-container {{ text-align: center; margin: 28px 0; }}
    .btn {{ background-color: #14b8a6; color: #ffffff; text-decoration: none; padding: 14px 32px; border-radius: 6px; font-size: 15px; font-weight: 600; display: inline-block; }}
    .expiry-note {{ font-size: 13px; color: #6b7280; text-align: center; }}
    .url-fallback {{ font-size: 12px; color: #9ca3af; word-break: break-all; margin-top: 16px; }}
...
<body>
  <div class="container">
    <div class="header"><h1>✉️ TrueGather</h1></div>
    <div class="body">
      <p>Bonjour <strong>{username}</strong>,</p>
      <p>Bienvenue sur TrueGather ! Pour activer votre compte, vérifiez votre adresse email en cliquant ci-dessous.</p>
      <div class="btn-container"><a href="{verify_url}" class="btn">Vérifier mon email</a></div>
      <p class="expiry-note">⏰ Ce lien expire dans <strong>15 minutes</strong>.</p>
      <p class="url-fallback">Si le bouton ne fonctionne pas, copiez ce lien :<br/>{verify_url}</p>
    </div>
    <div class="footer">© 2026 TrueGather · <a href="mailto:support@truegather.app">support@truegather.app</a></div>
  </div>
</body>
</html>"#,
        username = username,
